      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Novo Cargo" maxWidth="max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="input" type="number" value={form.salario_min} onChange={e => setForm(p => ({ ...p, salario_min: e.target.value }))} />
          </div>
          <div>
            <label className="label">Salário máximo</label>
            <input className="input" type="number" value={form.salario_max} onChange={e => setForm(p => ({ ...p, salario_max: e.target.value }))} />
          </div>
